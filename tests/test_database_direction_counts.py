...
def test_save_signal_persists_new_metadata_columns(tmp_path, monkeypatch):
    ...
    db.save_signal(
        ...
        setup_reason="EMA ribbon flip + Trend Bar confirmation",
    )

    row = db.get_pending_signals()[0]
    assert row["strategy_name"] == "Ribbon-Flip Trend-Bar Confirmation v1"
    assert row["score"] == 82.5
    assert row["rr"] == 1.72
    assert row["entry_timeframe"] == "5m"
    assert row["trend_timeframe"] == "15m"
    assert row["setup_reason"] == "EMA ribbon flip + Trend Bar confirmation"
