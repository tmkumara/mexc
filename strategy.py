...
Signals fire only through this stateful flow:
    detect_setup(symbol)
        1. Detect 15m market-structure bias.
        2. Confirm 1h trend, 5m EMA alignment, ATR, volume and BTC regime.
        3. Detect 5m liquidity sweep + displacement + order block.
        4. Save only setups close enough to the OB retest area.

    evaluate_pending_setup(setup)
        1. Monitor only high-quality waiting setups.
        2. Expire stale or far-away setups early.
        3. Wait for OB retest and confirmation candle.
        4. Revalidate filters before firing the Telegram signal.

The old EMA/CCI direct signal path is intentionally disabled.
"""
...
def _too_far_from_ob(price: float, zone_low: float, zone_high: float, atr_value: float, max_pct: float, max_atr: float) -> tuple[bool, float, float]:
...
def _htf_trend_ok(symbol: str, direction: str) -> tuple[bool, bool]:
    """Return (allowed, strong_agreement)."""
...
def detect_setup(symbol: str) -> dict | None:
    ...
    try:
        ...
        atr_ok, atr_value_pct, atr_value = _atr_status(completed)
        if not atr_ok:
        ...
        for displacement_pos in range(start, last_possible + 1):
            ...
            current_price = float(completed["close"].astype(float).iloc[-1])
            too_far, ob_distance_pct, ob_distance_atr = _too_far_from_ob(
                current_price,
                ob["zone_low"],
                ob["zone_high"],
                atr_value,
            ...
            if too_far:
                logger.info(
                    "[SETUP-REJECT] %s | price too far from OB distance=%.2f%% %.2fATR limit=%.2f%% %.2fATR",
                    symbol,
                    ob_distance_pct,
                    ob_distance_atr,
                    MAX_OB_DISTANCE_PCT,
            ...
            best_setup = {
                ...
                # non-persisted context for logs only; DB save ignores unknown keys
                "_atr_pct": atr_value_pct,
                "_ob_distance_pct": ob_distance_pct,
                "_ob_distance_atr": ob_distance_atr,
            }
        ...
        if best_setup:
            logger.info(
                ...
                f"dist={best_setup.get('_ob_distance_pct', 0):.2f}%/{best_setup.get('_ob_distance_atr', 0):.2f}ATR"
            )
...
def evaluate_pending_setup(setup: dict) -> tuple[str, Signal | None]:
    ...
    try:
        ...
        for _, row in recent.iterrows():
            ...
            if not valid_retest:
                _debug_wait(symbol, f"touched OB but confirmation failed close={float(row['close']):.6g} midpoint={midpoint:.6g}")
                continue
            ...
            return "FIRE", Signal(
                ...
                timeframe_summary=(
                    f"Hybrid SMC Pro | {TREND_TF} bias + {ENTRY_TF} sweep/OB retest | "
