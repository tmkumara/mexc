...
2. **Optional confirmation** (`SIGNAL_MODE=confirmed`, default on): the
   ...
   `strict` (VWAP + MTF) is explicitly out of scope, per earlier discussion.
...
(ccxt, SQLAlchemy, pydantic-settings, its own CLI). Per the "upgrade in
place" decision, none of that scaffolding is built — instead:
...
- Position sizing stays the bot's existing fixed-leverage/ROI% model
  (`LEVERAGE`, `MAX_SL_ROI_PCT`), not `ACCOUNT_BALANCE`/`RISK_PERCENT_PER_TRADE`.
- `SIGNAL_MODE=strict` (VWAP + multi-timeframe confirmation) is not built —
  confirmed-scope decision, keeps scan cost flat across the ~80-coin pool.
- Console-only notifications, pytest structure, and the phased dev sequence
  in the doc are not followed literally; this repo already has Telegram,
  a dashboard, and its own test suite, which are extended instead.
...
`fired_signal_id`) already matches PENDING→OPEN/EXPIRED/CANCELLED. Add two
columns via the existing `ALTER TABLE ADD COLUMN` pattern:
...
gain `tp2_price`/`tp3_price`. No other schema change to this table — it was
unused by any live code path, so there's no back-compat concern.
...
`no_ribbon_confirmation`, `no_ema200_confirmation`, plus existing
`stop_too_wide`, `rr_below_min`, `invalid_geometry`, `missing_data`,
`insufficient_history`, `candle_not_settled`, `error` (unchanged shape).
...
SIGNAL_MODE: str = os.getenv("SIGNAL_MODE", "confirmed")   # "original" | "confirmed"
...
- **`bot.py`**: `format_signal` rewritten to show all 3 targets and which
  fraction closes at each; new `notify_target_progress` (T1/T2 hit,
  reusing the `notify_v3_progress` reply-to-message pattern) sends "50%
  closed at T1, SL moved to breakeven" style updates; `notify_outcome`
  extended to label partial-ladder results (e.g. "T1+T2 HIT, BE STOP",
  "FULL TARGET (T1+T2+T3)", "STOPPED OUT") instead of only "TARGET
  HIT"/"STOP HIT". `cmd_status`'s config import/display block
  (`bot.py:208-223,244-246`) swaps `RIBBON_LOOKBACK_BARS`/
  `TREND_BAR_PAC_LENGTH`/`TARGET_ROI_PCT` for `SIGNAL_MODE`/
  `ENTRY_BUFFER_PCT`/`PENDING_SIGNAL_EXPIRY_CANDLES`.
- **`webui.py`**: `get_strategy_config()` (`webui.py:232-267`) drops the
  removed config names, adds the new ones; new pending-setups panel backed
  by `db.get_armed_setups()`; inline dashboard JS fields that read the
...
- **`database.py`**: add the `armed_setups.tp2_price/tp3_price` and
  `signals.tp3_price/tp2_hit_at` columns (as above); `save_armed_setup`'s
  INSERT gains the two new columns; new `mark_signal_tp2_hit(signal_id,
  hit_at)` mirroring the existing `mark_signal_tp1_hit`; `save_signal`
  gains optional `tp2_price=None, tp3_price=None` kwargs.
