...
async def cmd_status(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = "\n".join([
        ...
        "━━━━━━━━━━━━━━━━━━━━",
        f"State:      {_code(state)}",
        f"Strategy:   {_code('Trend Meter 3/3 + Stoch MTM Cross')}",
        f"TF:         {_code(STRATEGY_TF)}",
        f"Scan:       {_code(SETUP_SCAN_CRON_MINUTES)}",
        f"Trend M1:   {_code(f'EMA{TREND_EMA_FAST_1} > EMA{TREND_EMA_SLOW_1} for LONG')}",
        f"Trend M2:   {_code(f'EMA{TREND_EMA_FAST_2} > EMA{TREND_EMA_SLOW_2} for LONG')}",
        f"Trend M3:   {_code(f'Close > EMA{TREND_EMA_FILTER} for LONG')}",
        f"Stoch MTM:  {_code(f'length={STOCH_MTM_LENGTH}, LONG cross above {STOCH_MTM_LOWER:g}, SHORT cross below +{STOCH_MTM_UPPER:g}')}",
        f"SL:         {_code(f'ATR × {ATR_SL_MULTIPLIER:g}, limit {MIN_SL_PCT:g}%–{MAX_SL_PCT:g}%')}",
